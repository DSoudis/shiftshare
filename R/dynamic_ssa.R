...
get.Gk <- function(df, y.var ,region.var, year.var, sector.var, gregion.id, sector.id, start.year, end.year){
  require(dplyr)
  require(lazyeval)
  filter.criteria <- interp(~(between(y1, x1, x2)) & (y3 == x3) &
  ...
  temp.Gk <- df %>% filter_(filter.criteria) %>% select_(year.var, y.var)
...
get.Gik <- function(df, y.var ,region.var, year.var, sector.var, gregion.id, sector.id, start.year, end.year){
  ...
  temp.Gik <- df %>% filter_(filter.criteria) %>% select_(region.var, year.var ,sector.var, y.var)
  
  mutate.call <- interp(~((a - lag(a))/lag(a)), a = as.name(y.var))
  Gik <- temp.Gik %>% group_by_(sector.var) %>% mutate_(.dots = setNames(list(mutate.call), 'changeGik')) %>% select_(sector.var, year.var ,'changeGik')
  Gik
}
...
get.gik <- function(df, y.var, region.var, year.var, sector.var, gregion.id, sector.id, start.year, end.year){
  ...
  mutate.call <- interp(~((a - lag(a))/lag(a)), a = as.name(y.var))
  gik <- temp.gik %>% group_by_(region.var, sector.var) %>% mutate_(.dots = setNames(list(mutate.call), 'changegik')) %>%
    select_(sector.var, year.var ,'changegik') %>% arrange_(region.var, sector.var, year.var)
  gik
}
...
get.etk <- function(df, y.var, region.var, year.var, sector.var, gregion.id, sector.id, start.year, end.year){
  filter.criteria <- interp(~(between(y1, x1, x2)) & y3 != x3 & y4 != x4, .values = list(y1 = as.name(year.var), x1 = start.year, x2 = end.year,
                                                                                         y3 = as.name(sector.var), x3 = sector.id,
                                                                                         y4 = as.name(region.var), x4 = gregion.id))
  mutate.call <- interp(~lag(a, 1), a = as.name(y.var))
  
  etk <- df %>% filter_(filter.criteria) %>% select_(region.var, sector.var, year.var ,y.var) %>% arrange_(region.var, sector.var, year.var)
  etk <- etk %>% group_by_(region.var, sector.var) %>%  mutate_(.dots = setNames(list(mutate.call), 'laged'))
  etk
}
...
get.merged <- function(etk, Gik, gik, Gk, region.var, sector.var, year.var){
  merged <- left_join(etk, Gik, by = c(sector.var, year.var)) %>%
    left_join(.,gik, by = c(region.var, sector.var, year.var)) %>%
    left_join(.,Gk)
  merged
}
...
get.dssa <- function(merged, y.var, region.var, sector.var, year.var){
  ...
  results <- merged %>% group_by_(region.var, sector.var) %>% summarise(sum(NSik, na.rm = T),
                                                                        sum(IMik, na.rm = T),
                                                                        sum(RSik, na.rm = T))
